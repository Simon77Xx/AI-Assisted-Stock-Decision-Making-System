"""AI Stock Advisor — main orchestration service (upgraded with 8-factor + trading).

Ties together:
1. Market Data Service (real-time snapshots)
2. Scoring Engine (multi-stock ranking, 8-factor)
3. Financial Analyzer (PE/PB/ROE/growth data)
4. AI Decision Service (AI recommendations with actionable guidance)
5. Strategy-AI Comparison (consistency checks)
6. Beginner Output (plain-language formatting)
7. Trading Executor (同花顺 / simulated)
...
from ai_decision_service import AIRecommendation, format_financial_data_for_prompt, get_ai_recommendation
...
class AIStockAdvisor:
    ...
    def analyze_stock(
        ...
        """Full analysis of a single stock: score + financials + AI + consistency.
        ...
        # 3. Fetch financial metrics (非阻塞 — graceful degradation)
        fin_metrics: Optional[FinancialMetrics] = None
        fin_data_str = ""
        try:
            fin_metrics = get_financial_metrics(stock_code, stock_name or quote.stock_name)
            if fin_metrics and fin_metrics.has_data:
                fin_data_str = format_financial_data_for_prompt(fin_metrics)
        except Exception:
            fin_metrics = None

        # 4. Get AI recommendation (with financial context)
        ai_rec = get_ai_recommendation(
            ...
            financial_data_str=fin_data_str,
        )

        # 5. Update scoring with AI confidence
        update_ai_confidence(stock_code, ai_rec.confidence, market_snapshot.snapshot_id)

        # 6. Strategy-AI consistency
    ...
    def get_full_advisor_output(
        ...
        # Top stocks with additional score info
        top_stocks = []
        for r in ranking.rankings:
    ...
    def _decision_to_dict(
        ...
        return {
            "selected_stock_analysis": {
                ...
                # NEW: indicator signals from technical analysis
                "indicator_signals": scored.indicator_signals,
                "composite_indicator_score": scored.composite_indicator_score,
                # NEW: include financial metrics
                "financial_metrics": {
                    "pe_ttm": fin_metrics.pe_ttm if fin_metrics else None,
                    "roe": fin_metrics.roe if fin_metrics else None,
                    "net_profit_margin": fin_metrics.net_profit_margin if fin_metrics else None,
                    "gross_margin": fin_metrics.gross_margin if fin_metrics else None,
                    "revenue_growth": fin_metrics.revenue_growth if fin_metrics else None,
                    "net_profit_growth": fin_metrics.net_profit_growth if fin_metrics else None,
                    "debt_ratio": fin_metrics.debt_ratio if fin_metrics else None,
                    "eps": fin_metrics.eps if fin_metrics else None,
                } if fin_metrics and fin_metrics.has_data else None,
            },
            "ai_recommendation": {
                ...
                "entry_price_range": ai_rec.entry_price_range,
                "target_price": ai_rec.target_price,
                "stop_loss_price": ai_rec.stop_loss_price,
                "position_suggestion": ai_rec.position_suggestion,
                "position_ratio": ai_rec.position_ratio,
                "financial_summary": ai_rec.financial_summary,
                "trading_plan": ai_rec.trading_plan,
                "disclaimer": ai_rec.disclaimer,
            },
