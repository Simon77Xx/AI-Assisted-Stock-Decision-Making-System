...
# Proxy setting for AI model API calls (Anthropic, OpenAI, DeepSeek, etc.).
# The global HTTP_PROXY env var (set in api.py) covers akshare traffic.
# This is an additional layer specifically for urllib-based AI calls.
_PROXY_URL = os.environ.get("HTTP_PROXY") or os.environ.get("HTTPS_PROXY") or "http://127.0.0.1:7890"
_AI_PROXY_ENABLED = os.environ.get("AI_PROXY_ENABLED", "true").lower() in ("true", "1", "yes")


def _setup_urllib_proxy():
    """Configure urllib to route AI API calls through the proxy."""
    if not _AI_PROXY_ENABLED:
        return
...
# Initialize urllib proxy at import time (applies to _post_json calls)
_setup_urllib_proxy()
...
def _call_anthropic(system_prompt: str, user_prompt: str, model: str, max_tokens: int) -> str:
    ...
    if _AI_PROXY_ENABLED:
        try:
            import httpx
            http_client = httpx.Client(proxy=_PROXY_URL, timeout=DEFAULT_TIMEOUT)
        except ImportError:
            http_client = None
    else:
        http_client = None
